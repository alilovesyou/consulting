...
from database.db import add_user, update_user_profile, get_user_role, save_teacher_application, get_full_profile
...
# Asosiy menyu (O'quvchilar uchun)
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if role == 'teacher':
        ...
        await message.answer("👨‍🏫 Assalomu alaykum, Ustoz! Ish paneliga xush kelibsiz.", reply_markup=keyboard)
        return
        
    elif role == 'pending_teacher':
        kb = [[types.KeyboardButton(text="👤 Profil")]]
        keyboard = types.ReplyKeyboardMarkup(keyboard=kb, resize_keyboard=True)
        await message.answer("⏳ Sizning ustozlik arizangiz ma'muriyat tomonidan ko'rib chiqilmoqda. Iltimos kuting.", reply_markup=keyboard)
        return
        
    elif role == 'rejected_teacher':
        # Rad etilgan ustozga qaytadan topshirish yoki O'quvchi bo'lish imkoniyati
        kb = [
        ...
        # Ro'yxatdan o'tgan o'quvchi yana /start bossa, to'g'ridan to'g'ri menyuga tushadi
        await message.answer("🏠 Asosiy menyuga qaytdingiz.", reply_markup=student_main_menu())
        return

    elif role in ['admin', 'superadmin']:
        kb = [
            [types.KeyboardButton(text="➕ Guruh yaratish"), types.KeyboardButton(text="📚 Guruhlar ro'yxati")],
            [types.KeyboardButton(text="📊 Statistika")]
        ]
        keyboard = types.ReplyKeyboardMarkup(keyboard=kb, resize_keyboard=True)
        await message.answer("👨‍💻 Assalomu alaykum, Admin! Ish paneliga xush kelibsiz.", reply_markup=keyboard)
        return

    # AGAR UUMUMAN YANGI FOYDALANUVCHI BO'LSA:
    kb = [
        [types.KeyboardButton(text="👨‍🎓 O'quvchi bo'lib o'qish")],
        [types.KeyboardButton(text="👨‍🏫 Ustoz bo'lib ishlash")]
    ]
...
async def t_process_fio(message: types.Message, state: FSMContext):
    ...
    kb = [[types.KeyboardButton(text="📞 Raqamni yuborish", request_contact=True)], [types.KeyboardButton(text="🔙 Orqaga")]]
    keyboard = types.ReplyKeyboardMarkup(keyboard=kb, resize_keyboard=True)
...
# (Viloyat va Tuman tanlash Umumiy qismda joylashgan - ikkalasi uchun ham ishlaydi)
...
async def t_process_age(message: types.Message, state: FSMContext):
    ...
    # Tillarni tugma qilib chiqaramiz
...
async def t_process_lang(callback: types.CallbackQuery, state: FSMContext):
    lang_name = callback.data.split("_")[1]
...
async def t_process_exp(message: types.Message, state: FSMContext):
    ...
    data = await state.get_data() # Barcha kiritilgan ma'lumotlarni olamiz
    ...
    # Ustozga ko'rsatiladigan ma'lumotlar xulosasi
    summary = (
...
async def t_confirm_registration(message: types.Message, state: FSMContext, bot: Bot):
    ...
    if message.text == "✅ Tasdiqlash va Yuborish":
        ...
        # Bazaga 'pending_teacher' qilib saqlaymiz
        await save_teacher_application(
            message.from_user.id, data['fio'], data['phone'], data['region'], data['age'], data['lang'], data['cv_content']
        )
        ...
        if admin_id:
            ...
            builder = InlineKeyboardBuilder()
            builder.button(text="✅ Ishga qabul qilish", callback_data=AdminTeacherApproveCB(teacher_id=message.from_user.id))
            builder.button(text="❌ Rad etish", callback_data=AdminTeacherRejectCB(teacher_id=message.from_user.id))
            builder.adjust(2)
            ...
            if data['cv_type'] == 'text':
                ...
                await bot.send_photo(chat_id=admin_id, photo=data['cv_content'], caption=admin_text, parse_mode="HTML", reply_markup=builder.as_markup())
            elif data['cv_type'] == 'document':
                await bot.send_document(chat_id=admin_id, document=data['cv_content'], caption=admin_text, parse_mode="HTML", reply_markup=builder.as_markup())
...
# ==========================================
# 4. UMUMIY HUDUD TANLASH (USTOZ VA O'QUVCHI UCHUN)
# ==========================================
...
async def process_district_selection(callback: types.CallbackQuery, callback_data: DistrictCB, state: FSMContext):
    ...
    # Kim ro'yxatdan o'tayotganini tekshiramiz
    current_state = await state.get_state()
    if current_state == TeacherRegistration.region:
        await callback.message.answer(
...
# ==========================================
# 5. O'QUVCHI YOSH KELGANDA -> TASDIQLASHGA O'TISH
# ==========================================
...
async def process_age(message: types.Message, state: FSMContext):
    ...
    # Yoshni xotiraga saqlaymiz, lekin bazaga EMAS!
    await state.update_data(age=int(message.text))
    data = await state.get_data()
...
async def s_confirm_registration(message: types.Message, state: FSMContext):
    ...
    if message.text == "✅ Tasdiqlash":
        ...
        # Endi to'liq ishonch bilan bazaga saqlaymiz
        await update_user_profile(message.from_user.id, data['fio'], data['phone'], data['region'], data['age'])
        await state.clear()
...
async def show_profile(message: types.Message):
    ...
    if role == 'teacher':
        ...
        status_text = "👨‍💻 Ma'muriyat (Admin)"
    else:
        # Agar oddiy O'quvchi bo'lsa, uning to'lov holatini bazadan tekshiramiz
    ...
    # Xabarning eng pastiga Statusni qo'shamiz
